...
deactivate_plugin: ## deactivate all wordpress plugin
	docker exec -i bibcnrs-db-1 mysql --password='$(DB_PASSWORD)' wordpress -e "UPDATE wp_options SET option_value = '' WHERE option_name = 'active_plugins';"
...
wp-cli: ## allow to run dockerized wp-cli command
	docker exec bibcnrs_wordpress_1 wp --allow-root --path=/var/www/html $(COMMAND_ARGS)
...
build-docker: ## args: <version> build vxnexus-registry.intra.inist.fr:8083/bibcnrs/bibcnrs:<version> docker image default <version> to latest
ifdef COMMAND_ARGS
	docker build --no-cache -t 'vxnexus-registry.intra.inist.fr:8083/bibcnrs/bibcnrs:$(COMMAND_ARGS)' .
else
	docker build --no-cache -t 'vxnexus-registry.intra.inist.fr:8083/bibcnrs/bibcnrs:latest' .
endif
